...
class ConvEncoder(nn.Module):
    def __init__(self, in_channels=3, latent_dim=64, image_height=96, image_width=96, activation=nn.ReLU):
        super().__init__()
...
class ConvDecoder(nn.Module):
    ...
    def __init__(self, latent_dim, hidden_dim_rssm, out_channels=3, encoder_flat_size=23040, target_height=192, target_width=176, activation=nn.ReLU):
        ...
        # Créons un mini-encodeur temporaire pour obtenir ces dimensions (identique à ConvEncoder) :
        temp_conv1 = nn.Conv2d(out_channels, 32, kernel_size=4, stride=2, padding=1)
        temp_conv2 = nn.Conv2d(32, 64, kernel_size=4, stride=2, padding=1)
        temp_conv3 = nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1)
        temp_conv4 = nn.Conv2d(128, self.num_encoder_conv_filters, kernel_size=4, stride=2, padding=1)
        with torch.no_grad():
        ...
        # Les couches de déconvolution sont le strict inverse de l'encodeur (kernel 4, stride 2, padding 1)
        self.deconv1 = nn.ConvTranspose2d(self.num_encoder_conv_filters, 128, kernel_size=4, stride=2, padding=1)
        self.deconv2 = nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1)
        self.deconv3 = nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=1)
        self.deconv4 = nn.ConvTranspose2d(32, out_channels, kernel_size=4, stride=2, padding=1)
        print(f"[ConvDecoder] Initialized with exact reverse padding.")
...
class DreamerTrainer:
    def __init__(self, config):
        ...
        print(f"🚀 DreamerTrainer running on: {self.device}")
        self.config = config
        ...
        try:
            ...
            self.target_value_model = ValueModel(self.rnn_hidden_dim, self.latent_dim).to(self.device) # Target Critic
            self.target_value_model.load_state_dict(self.value_model.state_dict())
            self.target_value_model.eval()
            for p in self.target_value_model.parameters(): p.requires_grad = False
            self.actor = ActorModel(self.rnn_hidden_dim, self.latent_dim, self.action_dim).to(self.device)
        except RuntimeError as e:
            if "no kernel image is available" in str(e):
                print(f"\n{Fore.RED}❌ ERREUR CRITIQUE (Dreamer) : Votre version de PyTorch ne supporte pas l'architecture de votre GPU (sm_120 / RTX 50xx).")
    ...
    def train_step(self):
        ...
        # OPTIMISATION CRITIQUE CPU : Convertir le deque en liste UNE SEULE FOIS
        # au lieu de 128 fois, ce qui paralysait complètement le script à 150k !
        buffer_list = list(self.buffer)
        sequences = []
        for _ in range(batch_size):
            start_index = random.randint(0, len(buffer_list) - seq_len)
            sequences.append(buffer_list[start_index : start_index + seq_len])
        ...
        for t in range(seq_len -1): # On a besoin de o_t, a_t, r_t, o_{t+1}
            ...
            # STABILISATION : DreamerV2 KL Balancing
            # Protège contre la perte de gradient et empêche le "Posterior Collapse"
            free_nats = torch.tensor(3.0, device=self.device)
            
            # 1. Mettre à jour le prior vers le posterior (80% de l'effort)
            post_dist_sg = Independent(Normal(post_mean.detach(), post_std.detach()), 1)
            kl_prior = torch.max(kl_divergence(post_dist_sg, prior_dist), free_nats).mean()
            
            # 2. Mettre à jour le posterior vers le prior (20% de l'effort)
            prior_dist_sg = Independent(Normal(prior_mean.detach(), prior_std.detach()), 1)
            kl_post = torch.max(kl_divergence(posterior_dist, prior_dist_sg), free_nats).mean()
            ...
            # STABILISATION : Huber Loss (smooth_l1) pour gérer les grandes récompenses
            reward_loss = F.smooth_l1_loss(predicted_reward, reward_seq[:, t]) # prédit r_t
        ...
        for _ in range(self.imagination_horizon):
            # L'acteur a besoin de gradients pour apprendre, mais on détache l'état d'entrée
            # pour l'isoler complètement du modèle du monde
            h_in = current_h_imag.detach()
            z_in = current_z_imag.detach()

            # Action de l'acteur
            action_logits = self.actor(h_in, z_in)
            ...
            # Pas besoin de gradients pour la simulation du monde imaginaire ! (Gain énorme de perfs/RAM)
            with torch.no_grad():
        ...
        # On détache pour isoler le critique du modèle du monde
        imagined_values = self.value_model(all_imag_h.detach(), all_imag_z.detach()) # (H+1, (L-1)*B)
        
        # STABILISATION : Calculer les valeurs cibles avec le réseau cible (Target Network)
        with torch.no_grad():
            target_imagined_values = self.target_value_model(all_imag_h.detach(), all_imag_z.detach())
        ...
        lambda_returns, advantages = self.lambda_return(imagined_rewards, target_imagined_values, dones_imag, self.gamma, self.lambda_gae)
        # lambda_returns et advantages sont de taille (H, (L-1)*B)

        # --- STABILISATION : Normaliser les avantages ---
        # Empêche les gradients de l'acteur d'exploser si les retours imaginés deviennent très grands.
        # Cela contrôle la magnitude de la mise à jour de la politique.
        ...
        actor_loss = -(imagined_action_log_probs * advantages.detach()).mean() # .detach() est crucial
        ...
        total_actor_loss.backward() # Pas besoin de retain_graph car les graphes sont séparés par .detach()
        ...
        # STABILISATION : Huber Loss pour le Critique
        value_loss = F.smooth_l1_loss(imagined_values[:-1], lambda_returns.detach()) # detach returns
        ...
        # Mise à jour douce (Soft update) du réseau cible de la Valeur
        with torch.no_grad():
            tau = 0.02
    ...
    def load_model(self, path):
        ...
        try:
            ...
            if 'target_value_model' in data:
                self.target_value_model.load_state_dict(data['target_value_model'])
            else:
                self.target_value_model.load_state_dict(self.value_model.state_dict())
