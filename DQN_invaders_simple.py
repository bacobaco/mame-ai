...
def get_state():
    ...
    player_pos, psx, psy, saucer_pos, rx, ry, px, py, sx, sy, ay = list(
        map(int, response)
    )
    return np.array(
        [
...
def main():
    ...
    # 3*2 positions des bombes 1 position joueur et 1 pos soucoupe et 2 positions tir joueur + 1 pour ref_alien_y
    input_size = (10 + 1) * N
    nb_hidden_layer = 1
    hidden_size = 50
    ...
    for episode in range(num_episodes):
        ...
        while player_alive == 1:
            ...
            if debug >= 2:
                print(f"state={state}\nnext ={next_state}")
            reward += (next_state[-1] - state[-1] )* 20  # pénalité lorsque l'alien ref Y diminue (descente des aliens)
            sum_rewards += reward
