...
-- D�finitions des ports d'entr�e MAME (� adapter � votre configuration)
...
-- Ajoutez d'autres d�finitions de ports ici si n�cessaire
...
assert(ip, port)
print("Le serveur est en �coute sur le port " .. port)
...
local function accept_client()
    ...
    while not client do
        ...
        if new_client then
            ...
            socket.sleep(0.1) -- Attendre un peu avant de r�essayer (�vite une utilisation excessive du processeur)
        end
    end
end
accept_client()
...
local function main()
    ...
    if frame % 10000 == 0 then
        print("frame_in_game:" .. frame_in_game .. " - " .. tostring(nb_messages_total_in_game) ..
                  " messages traites soit " .. tostring(nb_messages_total_in_game *100// frame_in_game/100) .. " asks/frame")
    end
        ...
        if #messages_from_python > 0 then
            for _, message_from_python in ipairs(messages_from_python) do
                if message_from_python:find("execute") then
                    ...
                    if debug then
                        print("read from execute command:" .. message_from_python, command, value)
                    end
                    execute_command(command, tonumber(value))
                    table.insert(responses, "Command ack")
                ...
                elseif message_from_python:find("write_memory") then
                    ...
                    table.insert(responses, "Write memory ack")
